% extractSg146m12.m
%
% script to extract data from 2016 SeaGlider mission sg146m12
% 
...
sgd.opt = reshape(DG.optode_dphase_oxygen,ld,nd); % The dphase is missing for this cruise, so I had to use the data that have been computed internally
...
for i = 1:height(oxy_cal)
    [o2_comp.lag(i),ind_t] = min(abs(dived.date(3:end)-o2_comp.date(i))); ind_t = ind_t+2;
...
for i = 1:height(chl_cal)
    [chl_comp.lag(i),ind_t] = min(abs(dived.date(1:end)-chl_comp.date(i)));
...
o2_comp.sbe43 = NaN(height(oxy_cal),1);
for i = 1:height(oxy_cal)
    [o2_comp.lag(i),ind_t] = min(abs(diveu.date(2:end)-o2_comp.date(i)));  ind_t = ind_t +1;
...
subplot(5,2,[1 3 5])
contourf(dived.date,sgd.depth,sgd.s,32.75:0.05:35.4,'edgecolor','none')
hold on, contour(dived.date,sgd.depth,sgd.sig,[23.5 24.5 25.5],'k--'), hold off
hold on, plot(dived.date,mld003,'k'), hold off
set(gca,'ydir','rev','Fontsize',18)
ylabel('Depth (m)'), xlabel('Month/day 2016 (HST)')
datetick('x','mm/dd')
xlim([dived.date(1) dived.date(end)])
ylim([0 400])
caxis([34 35.3])
cb = colorbar; title(cb,'Salinity (g/kg)'); set(cb,'Fontsize',18)
subplot(5,2,[2 4 6])
contourf(dived.date,sgd.depth,sgd.opt,0:2:230,'edgecolor','none')
hold on, contour(dived.date,sgd.depth,sgd.sig,[23.5 24.5 25.5],'k--'), hold off
hold on, plot(dived.date,mld003,'k')
set(gca,'ydir','rev','Fontsize',18)
ylabel('Depth (m)'), xlabel('Month/day 2016 (HST)')
datetick('x','mm/dd')
xlim([dived.date(1) dived.date(end)])
...
subplot(5,2,[7 9])
contourf(dived.date,sgd.depth,sgd.chl2,0:0.01:3,'edgecolor','none')
hold on, contour(dived.date,sgd.depth,sgd.sig,[23.5 24.5 25.5],'k--'), hold off
hold on, plot(dived.date,mld003,'k')
set(gca,'ydir','rev','Fontsize',18)
ylabel('Depth (m)'), xlabel('Month/day 2016 (HST)')
...
ylim([0 200])
caxis([0 0.6])
cb = colorbar; title(cb,'Chlorophyll a'); set(cb,'Fontsize',18)
subplot(5,2,[8 10])
contourf(dived.date,sgd.depth,sgd.bbp660,0:2.5e-5:2e-3,'edgecolor','none')
hold on, contour(dived.date,sgd.depth,sgd.sig,[23.5 24.5 25.5],'k--'), hold off
hold on, plot(dived.date,mld003,'k')
set(gca,'ydir','rev','Fontsize',18)
ylabel('Depth (m)'), xlabel('Month/day 2016 (HST)')
...
ylim([0 200])
caxis([0 0.65e-3])
cb = colorbar; title(cb,'bbp660 (1/m)'); set(cb,'Fontsize',18)
colormap(jet(50))
...
hold on, surf(-sgd.lon,sgd.lat,repmat(sgd.depth,1,nd),sgd.chl2,'edgecolor','none','FaceAlpha',0.7),hold off
set(gca,'xdir','rev','zdir','rev','zlim',[0 200],'Color','none','Fontsize',18)
hold on,plot3(-dived.lon,dived.lat,mld003,'w','Linewidth',2), hold off
caxis([0 0.6])
colormap(jet)
shading interp

hold on, plot(-dived.lon,dived.lat,'k--','Linewidth',3), hold off
...
set(gca,'DataAspectRatio',[1 1 160],'xlim',[155 159],'ylim',[20.5 23.5])
grid on
